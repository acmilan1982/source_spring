...
public abstract class ReflectionUtils {
    ...
    /**
        ...
        Params:
          method – the method to invoke target – the target object to invoke the method on
    ...
    */
	...
	public static Object invokeMethod(Method method, @Nullable Object target) {
		return invokeMethod(method, target, EMPTY_OBJECT_ARRAY);                   // 见代码6
	}
    ...
    */
	...
	/**
		...
		Params:
		  clazz – the target class to analyze fc – the callback to invoke for each field
		Throws:
	...
	public static void doWithLocalFields(Class<?> clazz, FieldCallback fc) {
		// 获取当前类的所有字段
		for (Field field : getDeclaredFields(clazz)) {        // 见代码11
			try {
				// 每个字段调用参数指定的回调方法
				fc.doWith(field);
			}
	...
	/**
		This variant retrieves Class.getDeclaredFields() from a local cache in order to 
	...
	/**
		...
		Params:
		  clazz – the class to introspect mc – the callback to invoke for each method
		Throws:
		  IllegalStateException – if introspection fails
		Since:
		  4.2
