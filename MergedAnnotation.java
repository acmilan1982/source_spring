...
/**
	...
	MergedAnnotations集合中单个的聚合注解
	当前类表示一个注解，其属性值是“聚合”的(即属性值来源于不同的数据源)

	Attribute values may be accessed using the various get methods. 
	For example, to access an int attribute the getInt(String) method would be used.
    可以使用各种get方法访问属性值。
    例如，要访问int属性，将使用getInt（String）方法。
    ...
    请注意，访问时不会转换属性值。例如，如果基础属性是int，则不可能调用getString（String）。
    此规则的唯一例外是Class和Class[]值，它们可以分别作为String和String[]访问，以防止潜在的早期类初始化。

	
	If necessary, a MergedAnnotation can be synthesized back into an actual Annotation.
	如有必要，可以将MergedAnnotation合成为实际的Annotation。
