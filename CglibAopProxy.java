...
class CglibAopProxy implements AopProxy, Serializable {
	...
	private static final Map<Class<?>, Boolean> validatedClasses = new WeakHashMap<>();
						...
						/**
						    ...
						    AdvisedSupport持有代理对象所需的配置：
                  1. 目标对象：                     TargetSource targetSource = EMPTY_TARGET_SOURCE;		
                  2. 目标类实现的接口：             private List<Class> interfaces = new ArrayList<Class>();		
                  3. 应用于目标对象的所有Advisors:  private Advisor[] advisorArray = new Advisor[0];     
                  4. 继承于ProxyConfig的一些控制参数       
                  
                AdvisedSupport为当前类构造方法的参数：public Cglib2AopProxy(AdvisedSupport config)
	...
	protected Class<?>[] constructorArgTypes;
						/*
							  根据AdvisedSupport，创建AdvisedDispatcher(当前类的内部类)
							  AdvisedDispatcher实现了net.sf.cglib.proxy.Dispatcher接口
							  该Dispatcher对象的loadObject()方法返回AdvisedSupport
							  								  								  
							  因此，对代理对象上Advised接口方法的调用，实际上委托给该Dispatcher的loadObject()方法返回的对象，即AdvisedSupport
    ...
    /**
		...
		参数：AdvisedSupport，其持有代理对象所需的配置：
			1. 目标对象：                     TargetSource targetSource = EMPTY_TARGET_SOURCE;		
			2. 目标类实现的接口：              private List<Class> interfaces = new ArrayList<Class>();		
			3. 应用于目标对象的所有Advisors:   private Advisor[] advisorArray = new Advisor[0];     
			4. 继承于ProxyConfig的一些控制参数
										
		有了AdvisedSupport，就可以创建代理对象了 
    */
    // 构造方法1
	public CglibAopProxy(AdvisedSupport config) throws AopConfigException {
		...
		/*
			根据AdvisedSupport，创建Callback对象，实现类：AdvisedDispatcher(当前类的内部类)
			AdvisedDispatcher实现了net.sf.cglib.proxy.Dispatcher接口，而Dispatcher又是net.sf.cglib.proxy.Callback的子接口
			该Dispatcher对象的loadObject()方法返回AdvisedSupport
    ...
    /**
		...
		创建代理对象
		该方法的重点是Callback对象和CallbackFilter对象	
    */
    // 代码5
	@Override
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			...
			/*
				准备创建代理对象，使用cglib创建代理需提供4样东西：
				1. 代理对象的父类(即为目标类)
			*/	
			enhancer.setSuperclass(proxySuperClass);
			/*
				2. 代理对象实现的接口：								   
				   除了目标对象自己实现的接口以外，spring还提供了2个必须实现的接口：SpringProxy和Advised:
				   1. SpringProxy仅仅为一个标记接口，spring通过该接口能判断代理对象是否由spring创建
				   2. 而客户端通过Advised提供的方法，可以操作代理对象的Advisor(Advice)，用以增加或移除Advisor(Advice)
			...
			*/	
			...
			/*
				 3. 创建代理对象方法的回调接口
				    代理对象中，不同的接口方法，使用各自的Callback	
					这些Callback对象如下：		
					callbacks	Callback[7]  (id=44)	
						[0]	Cglib2AopProxy$DynamicAdvisedInterceptor  (id=38)	
						[1]	Cglib2AopProxy$StaticUnadvisedInterceptor  (id=41)	
						[2]	Cglib2AopProxy$SerializableNoOp  (id=47)	
						[3]	Cglib2AopProxy$StaticDispatcher  (id=42)	
						[4]	Cglib2AopProxy$AdvisedDispatcher  (id=49)	
						[5]	Cglib2AopProxy$EqualsInterceptor  (id=50)	
						[6]	Cglib2AopProxy$HashCodeInterceptor  (id=51)				
			*/	
			Callback[] callbacks = getCallbacks(rootClass);       // 见代码10

			// 以上这些Callback对象的实现类的Class对象
			Class<?>[] types = new Class<?>[callbacks.length];

            // 获取Callback对象的实现类的Class对象		
			for (int x = 0; x < types.length; x++) {
				types[x] = callbacks[x].getClass();
			}

			/*
				4. CallbackFilter，该对象指定了某个方法采用的Callback， 
				也就是说，Callback[]数组中的那个Callback作用于当前指定的方法上
				ProxyCallbackFilter为当前类的内部类，见其构造方法		
			*/	
			...
			return createProxyClassAndInstance(enhancer, callbacks);   // 见子类 ObjenesisCglibAopProxy
		}
    ...
    /**
		 创建代理对象的回调接口，这些Callback对象都是当前类的内部类
		 代理对象中，不同的接口方法，使用各自的Callback: org.springframework.cglib.proxy.Callback
    ...
    */
	...
	private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
		...
		boolean exposeProxy = this.advised.isExposeProxy();                 // 默认值: false
		boolean isFrozen = this.advised.isFrozen();                         // 默认值: false
		boolean isStatic = this.advised.getTargetSource().isStatic();       // 默认值: true

		// Choose an "aop" interceptor (used for AOP calls).
		Callback aopInterceptor = new DynamicAdvisedInterceptor(this.advised);         // 第一个 Callback
		...
		if (exposeProxy) {
			...
			targetInterceptor = (isStatic ?
					new StaticUnadvisedInterceptor(this.advised.getTargetSource().getTarget()) :    // 第二个Callback 
		...
		Callback targetDispatcher = (isStatic ?
				new StaticDispatcher(this.advised.getTargetSource().getTarget()) : new SerializableNoOp());   // 第三个Callback
