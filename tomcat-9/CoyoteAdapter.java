...
public class CoyoteAdapter implements Adapter {
    ...
    /*
        ...
        当前方法在Connector的initInternal()方法中被调用：adapter = new CoyoteAdapter(this);			    
    */
    //构造方法
    ...
    /*
        ...
        参数org.apache.coyote.Request和org.apache.coyote.Response并未实现任何接口
    */		
    //代码10	
    @Override
    public void service(org.apache.coyote.Request req, 
        ...
        if (request == null) {
									...
									/*
				              创建Request对象，实现类：org.apache.catalina.connector.Request，其持有org.apache.coyote.Request
				              其实现了javax.servlet.http.HttpServletRequest接口：
								        Request request = new Request();
								        request.setConnector(this);
								        return (request);				              
				              
				              创建Response对象，实现类：org.apache.catalina.connector.Response，其持有org.apache.coyote.Response
				              其实现了javax.servlet.http.HttpServletResponse接口：
        ...
        try {
            ...
            if (postParseSuccess) {
                ...
                /*
                    postParseRequest方法中，已经确定了处理当前请求的Container对象，包括Host，Context，Wrapper
                    因此，接下来由每个Container对象中的Pipeline的各个Valve来处理请求

                    connector.getContainer(): 从连接器中获取连接器关联的容器(Container),默认为: StandardEngine
                    getPipeline(): 获取容器的管道对象(Pipeline)，默认为: StandardPipeline 
                                    该方法定义在所有容器的父类：ContainerBase
                                    管道对象(Pipeline)也保存在该类的实例变量中：protected Pipeline pipeline = new StandardPipeline(this);
                                    见ContainerBase类代码6
                */	
    ...
    protected boolean postParseRequest(org.apache.coyote.Request req, Request request, org.apache.coyote.Response res,
        ...
        if (connector.getUseIPVHosts()) {
            ...
            // serverName是从请求头中获取的：Host: localhost:8080,
            // 此时，serverName是： localhost
        ...
        while (mapRequired) {
            /*
                    connector.getMapper()： 从连接器中获取Mapper对象：protected Mapper mapper = new Mapper();
                                            类型为：org.apache.tomcat.util.http.mapper.Mapper   
                                            
                    假设本机访问：  serverName：                 主机名，如：localhost，或IP地址: 127.0.0.1  
                                    decodedURI:                  当前请求的相对路径：/pureStruts/typeConverter
                                    request.getMappingData():    MappingData：org.apache.catalina.connector.Request的内部类，其属性尚未进行任何设置
                                                                                    当uri被解析以后，MappingData会持有:
                                                                                    1. 处理该请求的容器对象(Host,Context,wrapper),
                                                                                    2. 以及相关路径(contextPath,requestPath,wrapperPath,pathInfo)
                                                                                    稍后，tomcat会从MappingData中获取容器(Valve)来处理请求
                                                                                        并且，如果servlet需要创建RequestDispatcher，可能会使用路径来确定资源位置      
                                    
                    目前为止：已经有了主机名，从url中可以获取程序名(Context)及逻辑资源名
                    因此，map方法可以定位处理当前请求的Container对象，包括Host，Context，Wrapper   
                        
                    这些对象保存在org.apache.catalina.connector.Request的MappingData中
                    同时，org.apache.catalina.connector.Request中的实例属性直接引用着这些容器对象：protected Context context = null;
                                                                                                    protected Wrapper wrapper = null;  
                    
        根据servlet规范，把uri映射到servlet可分为以下几个步骤：
        1. Host匹配：Host通常在请求头中，与当前类 protected Host[] hosts = new Host[0] 中持有的所有HostName进行字符匹配即可
        
        2. Context匹配：context-path出现在uri中，匹配方式与host类似:
                        The Web application selected must have the the longest context path that matches the start of the request URL.  
                        
        3. Servlet匹配：uri使用逻辑映射来指定servlet(也可通过servlet name，但不安全，因此tomcat默认关闭这种方式)
                        uri中用来匹配servlet的路径又称servlet path，其等于uri - context-path - query string
                        依次使用servlet规范中定义的映射规则进行匹配，使用第一个被匹配的规则，并忽略剩下的规则  
                                                                        
                                                                        
        map方法执行完成之后，MappingData持有处理当前请求的：StandardContext，StandardWrapper
                                            以及当前请求的：requestPath，contextPath，wrapperPath     
                            Note: Host从Context中获取：return (Host) getContext().getParent();
                                    见org.apache.catalina.connector.Request类Host getHost()方法                           								                  
                    map方法见Mapper类代码12                     
            */	
            // This will map the the latest version by default
            connector.getService().getMapper().map(serverName,                  // localhost
                                                   decodedURI,                  // /springmvc_01_quickstart/save
                                                   version, 
