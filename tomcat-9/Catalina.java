...
public class Catalina {
   ...
   protected void parseServerXml(boolean start) {
	 ...
	 *  
        ...
        Digester提供了一系列已经实现的规则

        ObjectCreateRule - When the begin() method is called, this rule instantiates a new instance of a specified Java class, and pushes it on the stack. 
                           The class name to be used is defaulted according to a parameter passed to this rule's constructor, 
                           but can optionally be overridden by a classname passed via the specified attribute to the XML element being processed. 
                           When the end() method is called, the top object on the stack (presumably, the one we added in the begin() method) will be popped,and any reference to it (within the Digester) will be discarded.
                           当begin()被调用时，该规则会创建指定java类的一个实例，并压入栈，
                           默认的类名可由参数指定，也可由xml元素的属性来指定。
                           当end()被调用时，栈顶元素被弹出
        ...
        SetPropertiesRule - When the begin() method is called, the digester uses the standard Java Reflection API to identify any JavaBeans property setter methods 
                            (on the object at the top of the digester's stack) who have property names that match the attributes specified on this XML element, 
                            and then call them individually, passing the corresponding attribute values. 
                            These natural mappings can be overridden. This allows (for example) a class attribute to be mapped correctly. 
                            It is recommended that this feature should not be overused - in most cases, it's better to use the standard BeanInfo mechanism. 
                            A very common idiom is to define an object create rule, followed by a set properties rule, with the same element matching pattern.
                            This causes the creation of a new Java object, followed by "configuration" of that object's properties based on the attributes of the same XML element that created this object.
                            当begin()被调用时，当栈顶对象的属性名与xml属性名匹配时，digester使用反射找到这些属性的setter方法。
                            
        
        
        SetNextRule - When the end() method is called, the digester analyzes the next-to-top element on the stack, looking for a property setter method for a specified property. 
                      It then calls this method, passing the object at the top of the stack as an argument. 
                      This rule is commonly used to establish one-to-many relationships between the two objects, with the method name commonly being something like "addChild".
                      当end()被调用时，digester解析栈顶下的对象，寻找指定属性的setter方法，然后调用该方法，参数为栈顶元素
                      该规则通常用来建立父子关系
    ...
    protected Digester createStartDigester() {
        ...
        /*
            ...
            对于<Server>，如果没有指定className属性，那么创建默认实现：org.apache.catalina.core.StandardServer
        */        
        ...
        /*
            ...
            <Server>的属性值，设置到StandardServer对应属性中
         * 
         */
        ...
        /*
            ...
            对于<Server>，栈顶元素是StandardServer，下一个元素的是Catalina，
            指定Catalina的setServer方法，参数为栈顶元素StandardServer
        */
        ...
        // 对于<Server>下的<Service>，如果没有指定className属性，那么创建默认实现：org.apache.catalina.core.StandardService
        ...
        //栈顶元素是StandardService，下一个元素是StandardServer，Service是Server的属性
